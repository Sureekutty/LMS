IF OBJECT_ID ('speccs.SP_getFDLInterestAmount') IS NOT NULL
	DROP PROCEDURE speccs.SP_getFDLInterestAmount
GO

CREATE PROCEDURE speccs.SP_getFDLInterestAmount
...
--Calculation for principle of fdl start
   		SELECT LoanAccNo,TransactionDate,PayCode,Amount,P_I,ReceiptNo,ClosingBal INTO #tempdata FROM speccs.LoanTransactions WHERE LoanAccNo=@loanAccNo AND PayCode='L34'AND ClosingBal>0
	   	...
	   	BEGIN
	   		   ...
	   		   SELECT @dayDiff=(datediff(dd,@startDate,@month))%365
		   	     ...
		   	     BEGIN
			   	    ...
			   	    ELSE 
			   	    BEGIN  	
			   	     	SELECT  @intAmount=@intAmount+((Amount+@intAmount)*@intRate*@dayDiff)/36500 FROM #tempdata WHERE LoanAccNo=@loanAccNo AND TransactionDate= @startDate AND PayCode='L34'
			   	     END 
		   	     END 
	   			SELECT @intAmnt2=@intAmount+@intAmnt2
 			...
 			SELECT @dayDiff=(datediff(dd,@startDate,@month))/365
	   		   ...
	   		   SELECT @dayDiff=(datediff(dd,@startDate,@month))%365
		   	     ...
		   	     IF(((datediff(dd,@startDate,@month))/365)=0)
			   	    ...
			   	    ELSE 
			   	    BEGIN  	
			   	     	SELECT  @intAmount1=@intAmount1+((Amount+@intAmount1)*@intRate*@dayDiff)/36500 FROM speccs.LoanTransactions WHERE LoanAccNo=@loanAccNo AND TransactionDate= @startDate AND PayCode='L33'
			   	     END
		   	     END 
	   	--disbursement end
