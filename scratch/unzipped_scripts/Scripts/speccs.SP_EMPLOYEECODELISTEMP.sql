...
IF OBJECT_ID ('speccs.SP_EMPLOYEECODELISTEMP') IS NOT NULL
	DROP PROCEDURE speccs.SP_EMPLOYEECODELISTEMP
GO

CREATE  PROCEDURE speccs.SP_EMPLOYEECODELISTEMP
@Option  		VARCHAR (20),
@status CHAR(9),
@memCode VARCHAR(20)
...
*/
	...
	RETURN
	END
	IF (@Option="SOCIETYMEMEMP") 
	BEGIN
		SELECT MemEmpCode,MemName,MemAccNo FROM speccs.Members mem 
		WHERE mem.Status IN ('ACTIVE','SETTLED')AND mem.MemAccNo=@memCode
	...
	BEGIN
		SELECT MemEmpCode,MemName,MemAccNo FROM speccs.Members mem 
		WHERE mem.Status IN ('ACTIVE','SETTLED')
	...
	BEGIN
		SELECT DISTINCT(A.MemEmpCode+"-"+A.MemName) AS empcode FROM speccs.EmployeeData A
		 WHERE A.MemEmpCode NOT IN (SELECT MemEmpCode FROM speccs.Members WHERE Status='ACTIVE') 
