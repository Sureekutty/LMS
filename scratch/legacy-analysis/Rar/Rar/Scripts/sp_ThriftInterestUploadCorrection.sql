							...
							BEGIN
								...
								UPDATE speccs.LoanTransactions
				  				SET ClosingBal=ClosingBal-@Amount1
				   				WHERE LoanAccNo=@LoanAccNo AND ReceiptNo=@ReceiptNum AND PayCode IN('L23','L24','L26')AND P_I='P'
				    
			    				DELETE FROM #tempData78 WHERE LoanAccNo=@LoanAccNo AND ReceiptNo=@ReceiptNum 
							END
							
							
						SELECT TOP 1 @ClosingBalance=Min(ClosingBal) FROM speccs.LoanTransactions WHERE convert(DATE,TransactionDate)<=@UploadDate AND LoanAccNo=@RefNo AND PayCode IN('L23','L24','L26') ORDER BY TransactionDate desc
