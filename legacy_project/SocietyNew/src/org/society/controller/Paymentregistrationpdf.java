...
public class Paymentregistrationpdf {

	

	public static void getPDF(String purCode,String paymentfromdate,String paymenttodate,String header,String number) throws Exception {
		...
		outputStream = new FileOutputStream(new File("PaymentRegistration.pdf"));
		...
		document.add(p);
		document.add(enter);
		p = new Paragraph("                                                        Payments between "+paymentfromdate+" - "+paymenttodate+"                          Date: "+strDate+"    ",FontFactory.getFont(FontFactory.HELVETICA_BOLDOBLIQUE, 10, 0, new BaseColor(0, 0, 0)));
		...
		p.setSpacingAfter(5);
		document.add(p);
	    //String Fromdate,Todate,ReceiptNo,Receiptdate,MemeberAcc,RefId,Purpose;
	    //int Amount;
	   	...
	   	p = new Paragraph("PayVoucher No",  FontFactory.getFont(FontFactory.HELVETICA_BOLD, 10));
		p.setAlignment(Element.ALIGN_CENTER);
		table3.addCell(p);
		
		p = new Paragraph("Voucher Date",  FontFactory.getFont(FontFactory.HELVETICA_BOLD, 10));
		p.setAlignment(Element.ALIGN_CENTER);
		table3.addCell(p);
		...
		String sqlquery="speccs.SP_Prints 'PAYMENTREPORT','','"+purCode.split("-")[0]+"','"+paymentfromdate+"','"+paymenttodate+"'";
       ...
       while(rs3.next()) {
			...
			table7.addCell(new Phrase(rs3.getString("PayVoucherNo"),FontFactory.getFont(FontFactory.HELVETICA, 10, 0, new BaseColor(0, 0, 0))));
			table7.addCell(new Phrase(rs3.getString("VoucherDate"),FontFactory.getFont(FontFactory.HELVETICA, 10, 0, new BaseColor(0, 0, 0))));
