...
public class MISPaymentPDF {

public static void getPDF( HttpServletRequest request, HttpServletResponse response,String month,String year) throws Exception {
...
//		String strDate = new SimpleDateFormat("dd MMMMM, yyyy").format(new SimpleDateFormat("yyyy-MM-dd").parse(request.getParameter("idate").trim()));
		...
		date = new SimpleDateFormat("dd MMMMM, yyyy").format(new SimpleDateFormat("dd-MM-yyyy").parse(date));
		Document document = new Document(PageSize.A4);
		outputStream = new FileOutputStream(new File("MISPaymentsPDF.pdf"));
		...
		p = new Paragraph("MIS Payments for "+date,FontFactory.getFont(FontFactory.TIMES_BOLD,12));
		p.setAlignment(Element.ALIGN_CENTER);
		document.add(p);
		p=new Paragraph("\n");
		document.add(p);
		...
		for(int i=0;i<jsonArrayData.length();i++) {
			...
			p = new Paragraph(jobj.getString("Emp"), font6);			
			tables1.addCell(p);
			
			
			p = new Paragraph(jobj.getString("accNo"), font6);			
			tables1.addCell(p);
			
			p = new Paragraph(jobj.getInt("amount")+"", font6);			
			tables1.addCell(p);
