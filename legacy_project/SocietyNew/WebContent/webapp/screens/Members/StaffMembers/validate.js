$(document).ready(function() {
	...
	$('#phoneNumber,#bankNumber,#typeOfRelation,#aadharNumber,#sharesallot,#thriftSubAmt').change(function() {
		...
		if(id == 'thriftSubAmt'){
			...
			if(this.value < 200 ){
				alert('Amount should be equal or greater than 200');
				this.value = '';
				return;
			}
		}
...
function validateAadharNumber(aadharNo) {
	...
	if(aadharNo.length != 12 && aadharNo.length != 12){
		alert('Aadhar Number length should be 11 digits');
		return; 
	}
	return true;
}
...
function svalidateSave() {
	...
	if(basicpay == '' || basicpay == undefined){
		alert('Enter Basicpay');
		return;
	}
	var Dateofbirth = $('#Dateofbirth').val();
	if(Dateofbirth == '' || Dateofbirth == undefined){
		alert('Select DateOfBirth');
		return;
	}
	...
	if(careOf == '' || careOf == undefined){
		alert('Enter Parent or Husband ');
		return;
	}
	var officeno = $('#officeno').val();
	if(officeno == '' || officeno == undefined){
		alert('Enter officeNo ');
		return;
	}
